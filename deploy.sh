...
tar cvzf $APP_NAME.tar.gz *.js *.css *.html
...
ls *.css *.html *.js -1b | awk -v appname=$APP_NAME -v path=$REMOTE_PATH -v docker_image=$DOCKER_IMAGE -v docker_path=$DOCKER_PATH '{print "docker cp " path "/" appname "/" $1 " " docker_image ":" docker_path "/" appname "/" $1}' >> remote_deploy.sh
